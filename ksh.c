...
kvalue( FILE *fp, const jvalue *j, bool nested, unsigned depth )
{
	...
	/* When we dump an object, we dump the "simpler" types at the top,
	 * and the more complex ones at the bottom.  This loop used to
	   simply be the following, which dumped in tree order.
	...
	*/
	...
	for( jvalue **jj = j->u.v; *jj; ++jj ) {
		jd = (*jj)->d;
		if( jd == jnull || jd == jtrue || jd == jfalse )
			kvalue( fp, *jj, false, depth+1 );
	}
	for( jvalue **jj = j->u.v; *jj; ++jj ) {
		jd = (*jj)->d;
		if( jd == jstring || jd == jnumber || jd == jint || jd == jreal)
			kvalue( fp, *jj, false, depth+1 );
	}
	for( jvalue **jj = j->u.v; *jj; ++jj ) {
		jd = (*jj)->d;
